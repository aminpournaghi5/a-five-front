...
function EditProgram() {
  ...
  const exerciselist = useSelector(
    (state: any) => state.editExerciseList.exerciselist
  );
  const UpdateExerciseList = async () => {
    ...
    try {
      ...
      // ارسال درخواست PUT برای بروزرسانی تمرین
      const response = await axiosInstance.put(
        `/api/exerciselist/update/${params.id}`, // استفاده از پارامتر ID در مسیر
        payload // ارسال داده‌ها به سرور
      );

      // بررسی پاسخ موفقیت‌آمیز
      if (response.status === 200) {
        // پس از بروزرسانی موفقیت‌آمیز، کاربر را به صفحه مربوطه هدایت می‌کنیم
        window.location.href = `/programming/${params.id}`;
      }
    } catch (error: any) {
